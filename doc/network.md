...
The [Canonical snap](https://snapcraft.io/tailscale) is strictly confined. That means:
...
Tailscale is the network path only. It does not install OSes, shrink partitions, or pick GRUB entries. A second-OS install still needs a local console for the first boot.
